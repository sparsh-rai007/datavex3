...
export async function generateFromUrl(
  ...
  console.log("\n" + "=".repeat(60));
  console.log(`🚀 https://en.wikipedia.org/wiki/Mode_%28statistics%29 ${url}`);
  ...
  const systemPrompt = `You are an expert tech writer who creates engaging, human-sounding blog posts. Your writing is conversational, relatable, and easy to read.

Rules you MUST follow:
1. Output clean Markdown only.
2. Start with a compelling H1 title (# Title) on the very first line.
3. TONE & STYLE: Write in a natural, conversational, and humanized tone. Strictly avoid overused AI clichés like "In today's fast-paced digital landscape" or "Delve into". Use active voice and short paragraphs.
4. Keep ALL code blocks exactly as they appear.
5. CRITICAL IMAGE RULE: ONLY keep images that are highly relevant to the technical topic. 
6. CRUCIAL CITATION RULE: Cite your sources at the paragraph level. Example format: ...end of paragraph text. ([Source](${url}))
7. Do not wrap your response in HTML tags. Include double line breaks (\n\n) before AND after every heading, list, and paragraph.`;
...
export async function generateFromKeyword(
  ...
  const systemPrompt = `You are an expert tech writer who synthesizes multiple research sources into cohesive, human-sounding blog posts. Your writing is conversational, relatable, and easy to read.

Rules you MUST follow:
1. Output clean Markdown only.
2. Start with a compelling H1 title (# Title) on the very first line.
3. TONE & STYLE: Write in a natural, conversational, and humanized tone. Strictly avoid overused AI clichés like "In today's fast-paced digital landscape" or "Delve into".
4. DO NOT copy content verbatim. Synthesize, clarify, and add editorial insight.
5. Keep useful code blocks exactly as they appear.
6. CRITICAL IMAGE RULE: ONLY keep images that are highly relevant to the technical topic.
7. CRUCIAL CITATION RULE: You must cite your sources at the paragraph level. Example format: ...end of paragraph text. ([Source](URL))
8. Include double line breaks (\n\n) before AND after every heading, list, and paragraph.`;
