...
// If the instruction contains a URL, the page is scraped and its content
// is passed as reference material to the AI for context-aware rewriting.
// ---------------------------------------------------------------------------

// URL regex to detect links in the instruction
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    // ── Existing blog context (truncated to stay within token limits) ──
    ...
    // ── Detect URLs in the instruction ──
    const detectedUrls = instruction.match(URL_REGEX) || [];
    const cleanInstruction = instruction.replace(URL_REGEX, "").trim();
    ...
    // ── Scrape all detected URLs in parallel ──
    let scrapedContext = "";
    if (hasUrls) {
      const uniqueUrls = [...new Set(detectedUrls.map(u => u.trim()))];
      console.log(`🔗 [Edit Snippet] Scraping ${uniqueUrls.length} URL(s)...`);
      const scrapeResults = await Promise.all(uniqueUrls.map(scrapeUrl));

      const sections = scrapeResults
        .map((content, i) => {
          if (!content || content.trim().length < 30) return null;
          // Cap each scraped page at 4000 chars to stay within token limits
      ...
      if (sections.length > 0) {
        ...
        console.log(`📄 [Edit Snippet] Total scraped context: ${scrapedContext.length} chars`);
      } else {
        console.warn(`⚠️  [Edit Snippet] All URL scrapes returned empty — proceeding without context`);
      }
    }

    // ── De-duplication context block ──
    ...
    if (isGenerateMode) {
      // ─── GENERATE MODE: No text selected ───
      if (hasUrls && scrapedContext) {
        if (cleanInstruction) {
          // URL + specific instruction → do exactly what they asked using scraped data
...
${scrapedContext}${dedupeBlock}`;
          ...
          // URL only, no instruction → pick the most relevant section
...
${scrapedContext}${dedupeBlock}`;
        ...
        // No URL, just a text instruction → do exactly what they asked
...
- Do NOT repeat anything from the existing blog content.${dedupeBlock}`;
      ...
      // ─── REWRITE MODE: Text is selected ───
      if (hasUrls && scrapedContext) {
        if (cleanInstruction) {
          // URL + instruction + selected text → rewrite per instruction using scraped data
...
${scrapedContext}${dedupeBlock}`;
          ...
          // URL + selected text, no instruction → update selected text with scraped info
...
${scrapedContext}${dedupeBlock}`;
        ...
        // No URL, just instruction + selected text → do exactly what they said
        systemPrompt = `You are a precise inline editor. The user has selected text and wants it modified.
