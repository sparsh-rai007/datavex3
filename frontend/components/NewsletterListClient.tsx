...
export default function NewsletterListClient({
  ...
  const resolvedBasePath = basePath || (contentType === 'blog' ? '/blog' : '/newsletter');
  ...
  const handleReadMore = (post: BlogPost) => {
    if (post.external_url) {
      ...
      router.push(`${resolvedBasePath}/${post.slug}`);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {currentPage === 1 && activeCategory === 'All Records' && !searchQuery && featuredPost && (
            ...
            >
              <div
                ...
                className={`group cursor-pointer relative grid gap-12 bg-white rounded-[4rem] overflow-hidden border border-slate-100 shadow-2xl shadow-slate-200/40 p-4 md:p-8 ${featuredPost.featured_image ? 'grid-cols-1 lg:grid-cols-12' : 'grid-cols-1'
                  }`}
              >
                ...
                <div className={`flex flex-col justify-center py-10 px-6 ${featuredPost.featured_image ? 'lg:col-span-6' : 'max-w-4xl mx-auto text-center items-center'}`}>
                  ...
                  <div className={`flex items-center justify-between w-full pt-10 border-t border-slate-50 ${!featuredPost.featured_image ? 'max-w-2xl' : ''}`}>
                    <div className="flex items-center gap-4 py-4">
                      {/* Author/Date references removed */}
                    </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-4 xl:pb-0 no-scrollbar">
              {CATEGORIES.map((cat) => (
                <button
                  ...
                  className={`px-8 py-4 rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] whitespace-nowrap transition-all border-2 ${activeCategory === cat
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-4 mt-12 pb-20">
              <button
                ...
                className="px-6 py-3 bg-white border border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-primary-600 disabled:opacity-30 disabled:pointer-events-none transition-all shadow-sm flex items-center gap-2"
              >
          ...
          {paginatedBlogs.length === 0 && (
            ...
            >
              ...
              <button
                onClick={() => { setActiveCategory('All Records'); }}
