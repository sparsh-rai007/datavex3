...
function cleanMarkdown(rawText: string, stripReferences: boolean) {
  if (!rawText) return '';
  ...
  if (stripReferences) {
    // Aggressive removal of trailing references section
    cleaned = cleaned.replace(/(\r?\n|^)[ \t]*#*[ \t]*(References|Sources|Citations|Bibliography|Related Links|External Nodes)\b[\s\S]*$/gi, '');
    // Remove any leftover trailing bulleted links
    ...
    <div className="prose prose-slate prose-lg max-w-none prose-headings:font-black prose-strong:text-slate-900 markdown-body dark:prose-invert">
      <ReactMarkdown
        ...
        components={{
          ...
          a: ({ node, ...props }) => {
            if (hideLinks) {
              if (!props.children || String(props.children).trim() === '' || String(props.children) === '""') return null;
              return <span className="text-slate-900 border-b border-dotted border-slate-400 cursor-default">{props.children}</span>;
            }
